    ...
    if(M==32){
        ...
        for (i = 0; i < N; i+=8)
        {
            for (j = 0; j < M; j+=8) {
                ...
                //row by row
                //divide into submatrix to do the operation
                for (int r = i; r < i+8;r++){
                    int value =0;
                    int pos = -1;
                    //it can get 343 cache misses, it will be a miss to the cache when diagonse has equal amounts of the block
                    for (int c = j; c < j+8; c++)
                    {
                        ...
                        if (r != c)
                        {
                            B[c][r] = A[r][c]; // Transpose non-diagonal elements
                        } else {
                            //diagnose save until the block is finish reading
                            value = A[r][c];
                            pos = r;
                            // B[c][r] = A[r][c]; // Keep diagonal elements as is
                        }
                    ...
                    // //read the entire row, store them using varaible as a cache line and write to the B
                    // int r1 = A[r][j];
                    // int r2 = A[r][j+1];
                    // int r3 = A[r][j + 2];
                    // int r4 = A[r][j + 3];
